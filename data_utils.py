...
class TrainDataset(torch.utils.data.Dataset):
    ...
    def weighted_f1(self, predictions):
        """
        Weighted F1 score.
        """
        weighted_f1_sum = 0.0
        ...
        for c in range(self.num_classes):
            tp = np.sum((predictions == c) & (self.seg_label == c))
            fp = np.sum((predictions == c) & (self.seg_label != c))
            fn = np.sum((predictions != c) & (self.seg_label == c))

            support = np.sum(self.seg_label == c)
            if support == 0:
                continue
            ...
            weighted_f1_sum += (support / total_samples) * f1_c
...
class TestDataset(torch.utils.data.Dataset):
    ...
    def weighted_f1(self, utt_preds):
        """
        Calculate Weighted F1 Score given the utterance-level predictions.

        Parameters
        ----------
        utt_preds : ndarray
            Processed utterance-level predictions.
        ...
        weighted_f1_sum = 0.0
        total_samples = self.n_actual_samples

        for c in range(self.num_classes):

            # True Positives, False Positives, False Negatives
            tp = np.sum((utt_preds == c) & (self.actual_target == c))
            fp = np.sum((utt_preds == c) & (self.actual_target != c))
            fn = np.sum((utt_preds != c) & (self.actual_target == c))

            # Support = true samples of class c
            support = np.sum(self.actual_target == c)
            if support == 0:
                continue  # skip class with no samples
        ...
        return weighted_f1_sum
    def confusion_matrix(self,utt_preds,dataset_type):
        '''
        修改点！！！
        IEMOCAP数据集：'ang':0,'sad':1,'hap':2,'exc':2,'neu':3
        "ang": conf[:, 0], "sad": conf[:, 1],
                             "hap": conf[:, 2], "neu": conf[:, 3]

        EMODB数据集：'ang':0,'sad':1, 'hap':2,'neu':3,"fea":4,'dis': 5,'bor':6
        "ang": conf[:, 0], "sad": conf[:, 1], "hap": conf[:, 2], "neu": conf[:, 3],
        "fea": conf[:, 4], "dis": conf[:, 5], "bor": conf[:, 6]
        ...
        "neu": conf[:, 0], "hap": conf[:, 1], "sad": conf[:, 2], "ang": conf[:, 3],
        "fea": conf[:, 4], "dis": conf[:, 5], "sur": conf[:, 6], "cal": conf[:, 7]
        '''
        #输出混淆矩阵。
        conf = confusion_matrix(self.actual_target, utt_preds, labels=list(range(self.num_classes)))
        ...
        if dataset_type == "IEMOCAP":
            ...
            conf_fmt = pd.DataFrame({"ang": conf[:, 0], "sad": conf[:, 1], "hap": conf[:, 2], "neu": conf[:, 3],
                                    "fea": conf[:, 4], "dis": conf[:, 5], "bor": conf[:, 6]})
        elif dataset_type == "RAVDESS":
            conf_fmt = pd.DataFrame({"neu": conf[:, 0], "cal": conf[:, 1], "hap": conf[:, 2], "sad": conf[:, 3],
                                    "ang": conf[:, 4], "fea": conf[:, 5], "dis": conf[:, 6], "sur": conf[:, 7]})
        elif dataset_type == "MELD":
            conf_fmt = pd.DataFrame({"neu": conf[:, 0], "hap": conf[:, 1], "sad": conf[:, 2], "ang": conf[:, 3],
                                    "sur": conf[:, 4], "fea": conf[:, 5], "dis": conf[:, 6]})
...
class SERDataset:
    ...
    """
    def __init__(self, features_data, dataset_type,
